...
import { WebcamInfo } from "@/lib/utils";
  ...
  export function CamSelector({ devices, handleDevices, className=`` } : { devices:WebcamInfo[], handleDevices: (device: WebcamInfo) => void, className:string}) {
      return (
          <DropdownMenu>
          ...
          <DropdownMenuContent>
            ...
            {/* <DropdownMenuSeparator /> */}
            {
              ...
              :
              devices.map((device, key) => <DropdownMenuItem key = {key} onClick={() => handleDevices(device)}>{ device.CamLabel }</DropdownMenuItem>)
            }
          </DropdownMenuContent>
        </DropdownMenu>
      )
